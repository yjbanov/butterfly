// This is a direct port of Flutter `framework.dart` source code, used to
// assess the feasibility of the port. With minimal changes I got this to
// compile on top of `dart:html`.
...
abstract class ParentDataWidget<T extends RenderObjectWidget> extends _ProxyComponent {
  ...
  void applyParentData(html.Node renderObject);
}
...
/// which wrap [html.Node]s, which provide the actual rendering of the
/// application.
abstract class RenderObjectWidget extends Widget {
  ...
  /// createRenderObject().
  void updateRenderObject(html.Node renderObject, RenderObjectWidget oldWidget) { }

  void didUnmountRenderObject(html.Node renderObject) { }
}
...
abstract class BuildContext {
  Widget get widget;
  html.Node findRenderObject();
  InheritedWidget inheritFromWidgetOfExactType(Type targetType);
  Widget ancestorWidgetOfExactType(Type targetType);
  State ancestorStateOfType(TypeMatcher matcher);
  html.Node ancestorRenderObjectOfType(TypeMatcher matcher);
...
abstract class Element<T extends Widget> implements BuildContext {
  ...
  html.Node findRenderObject() => renderObject;
...
abstract class RenderObjectElement<T extends RenderObjectWidget> extends BuildableElement<T> {
  ...
  /// The underlying [html.Node] for this element
  html.Node get renderObject => _renderObject;
  final html.Node _renderObject;
  ...
  void debugUpdateRenderObjectOwner() {
    debugOwner[_renderObject] = debugGetOwnershipChain(10);
  }
  ...
  void insertChildRenderObject(html.Node child, dynamic slot);
  void moveChildRenderObject(html.Node child, dynamic slot);
  void removeChildRenderObject(html.Node child);
...
class MultiChildRenderObjectElement<T extends MultiChildRenderObjectWidget> extends RenderObjectElement<T> {
  ...
  void insertChildRenderObject(html.Node child, Element slot) {
    final html.Node renderObject = this.renderObject;
    html.Node after = slot?.renderObject;
    if (after == null) {
      if (renderObject.childNodes.isNotEmpty) {
        renderObject.insertBefore(child, renderObject.childNodes.first);
      } else {
        renderObject.append(child);
      }
    } else {
      renderObject.insertBefore(child, after.nextNode);
    }
    assert(renderObject == this.renderObject);
  }

  void moveChildRenderObject(html.Node child, Element slot) {
    final html.Node renderObject = this.renderObject;
    child.remove();
